...
std::string getCurrentDirectory () {
  size_t bufSize = ::GetCurrentDirectoryA(0, NULL);
...
std::string expandFilePath (const std::string &filePath) {
  size_t bufSize = ::GetFullPathNameA(filePath.c_str(), 0, NULL, NULL);
...
bool compareCaseInsensitively (const std::string &a, const std::string &b) {
...
void sortCaseInsensitively (std::vector<std::string> &list) {
  std::sort(list.begin(), list.end(), compareCaseInsensitively);
}
