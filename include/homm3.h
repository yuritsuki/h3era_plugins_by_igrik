...
// проверка на пустую строку символов
_bool_ IsNullOrEmptyString(char* input)
{
  if (input)
  {
    string name = input;
...
// проверка на null указатель на массив символов
_bool_ IsNullString(char* input)
{
...
NOALIGN struct _TagRect_ // Size = 8
{
  _int_ left;
  _int_ top;
  _int_ right;
...
NOALIGN struct _SoundNode_ // Size = 8
{
...
NOALIGN struct _Resources_
{
  ...
  char* GetNameByIndex(_int_ index) { return o_RESTYPES_TXT->GetString(index+1); }
  _int_ GetByIndex(_int_ index) { return *(int*)((_dword_)&this->wood +index*4); }
  _bool8_ EnoughResources(_Resources_* cost);
  void RemoveResources(_Resources_* cost);
  _Resources_* GetUpradeCreatureCost(_int_ type, _int_ typeUpgrade, _int_ count)
   { return CALL_4(_Resources_*, __fastcall, 0x54EB50, type, typeUpgrade, count, this); }
};

NOALIGN struct _Position_
{
  _int_ xyz;
};
...
#define GetCreatureUpradeCost(getCost, Type, UpType, Count) CALL_4(_int_, __fastcall, 0x54EB50, (_int_)Type, (_int_)UpType, (_int_)Count, _Resources_* getCost)
...
NOALIGN struct _GameMgr_: _Struct_
{
 ...
 inline _Town_* GetTownByMapPoint(_int_ mapPoint)
 {
  _int_ townId = CALL_4(_int_, __thiscall, 0x4BB530,this, b_unpack_x(mapPoint), b_unpack_y(mapPoint), b_unpack_z(mapPoint));
...
NOALIGN struct _TownMgr_: _Struct_
{
 _Manager_ mgr;
 _Town_* town;                  // +56   0x38
 _dword_ panorama;              // +50   0x3C
 _dword_ monPix[7];             // +64   0x40
 _dword_ nobjects[44];          // +92   0x5C
 _int32_ numObjects;            // +268  0x10C
 _int32_ loadedTownType;        // +272  0x110
 _Dlg_* menu;                   // +276  0x114
 _Dlg_* dlg;                    // +280  0x118
 _GarrisonBar_* garribar_up;    // +284  garrisonStrip
 _GarrisonBar_* garribar_down;  // +288  heroStrip
 _GarrisonBar_* garribar_sel;   // +292  currStrip
 _int_ garribar_slot_index_sel; // +296  currIndex
 _GarrisonBar_* garribar_src;   // +300  srcStrip
 _int_ garribar_slot_index_src; // +304  srcIndex
 _GarrisonBar_* garribar_dst;   // +308  destStrip
 _int_ garribar_slot_index_dst; // +312  destIndex
 _ptr_ townBank;                // +316  resources_bar
 _ptr_ townPopupBank;           // +320
 _char_ statusbar_text[80];     // +324  0x144
 _int_ lastHover;               // +404  0x194
 _int_ lastQualifier;           // +404  0x198
 _int_ command_code;            // +412  0x19C
 _int64_ canBuyMask;            // +416  0x1A0
 _int64_ canBuildMask;          // +424  0x1A8
 _int_ pNetMsgHandler;          // +432  0x1B0
 _int_ pNetMsgHandlerSave;      // +436  0x1B4
 _dword_ objToBuild;            // +440  0x1B8
 _dword_ dialogWindow;          // +444  0x1BC
 _int_ multiWin;                // +448  0x1C0
 _int_ divideStatus;            // +452  0x1C4
 _int_ recruitSelected;         // +456  0x1C8
 _byte_ currentDwellingIDOff[7];// +456  0x1CC
 _byte_ visible; // igrik var!! // +467  0x1D3
 _int_ align;                   // +468  0x1D4
                            // end +472  0x1D8
...
NOALIGN struct _AdvMgr_: _Struct_ // size: 952 bytes = 0x3B8
{
  ...
  _List_<_ptr_> CachedGraphics; // +208 0xD0
  _Def_* monAttackSprites;    // +224 0xE0
  _Position_ xyz;             // +228 0xE4
  _Position_ xyzPrevious;     // +232 0xE8
  ...
  _int_ InfoPanelOverride;    // +920 0x398 (enum EBottomViewType)
  _int_ InfoPanelOverrideEndTime;  // +924 0x39C
  _int_ InfoPanelResource;    // +928 0x3A0
  _int_ InfoPanelResourceQty; // +932 0x3A4
  _HStr_ BottomViewText;      // +936 0x3A8 (std::string)
  // end: +952 0x3B8
...
NOALIGN struct _AdvMgrSave_: _Struct_
{
 _Manager_ mgr; // 0...56
