...
void CreateNewBattleLogDlg(_BattleMgr_ *bm)
{
    ...
    if (lines)
    {
        ...
        dlg->AddItem(scrollText =
                         _DlgScrollableText_::Create((char *)text.c_str(), 40, 38, x - 78, y - 120, n_MedFont, 1, 0));
...
int PrepareRemovedArmyMessage(const int side, char *const buffer)
{
    ...
    _Army_ *removedArmy = *reinterpret_cast<_Army_ **>(0x076C9FE + 1);
    std::string creatureNames;

    // итерируем отнятых существ
    for (size_t i = 0; i < 7; i++)
    {
        if (const int removedNum = removedArmy[side].count[i])
        {
            sprintf(myString3, "%d %s, ", removedNum, o_CreatureInfo->GetName(removedArmy[side].type[i], removedNum));
...

{
    if (!o_BattleMgr->isTactics && !o_BattleMgr->IsHiddenBattle())
    {
        ...
        for (size_t i = 0; i < 2; i++)
        {
            ...
            {
                char *text = i ? GetEraJSON("wnd.combat.army_decreased_attacker")
                               : GetEraJSON("wnd.combat.army_decreased_defender");
...
// пропускаем отображение сообщения о присоединении армии к герою командиром Инферно
DWORD charmArmyTextPtr = 0;
_LHF_(WoG_NPC_JoinArmyMessage)
{
    charmArmyTextPtr = c->eax;
    // блокируем отображение сообщения об уменьшении армии в лог битвы
    return SKIP_DEFAULT;
}
_LHF_(MineDlg_Ctor)
{
    if (charmArmyTextPtr)
    {
        c->esi = charmArmyTextPtr;
        charmArmyTextPtr = 0;
        // блокируем отображение сообщения о присоединении армии к герою командиром Инферно
        return SKIP_DEFAULT;
    }
    return EXEC_DEFAULT;
}
...
void DlgBattleLog(PatcherInstance *_PI)
{
    ...
    // блокируем оригинальную функцию показа сообщения о возврате армии герою;
    _PI->WriteJmp(0x076C695, 0x076C6DB);

    // пропускаем отображение сообщения о присоединении армии к герою командиром Инферно
    _PI->WriteLoHook(0x05D0FB4, MineDlg_Ctor);
    _PI->WriteLoHook(0x076CC69, WoG_NPC_JoinArmyMessage); // attacker inferno commander message box
    _PI->WriteLoHook(0x076CE17, WoG_NPC_JoinArmyMessage); // defender inferno commander message box
}
